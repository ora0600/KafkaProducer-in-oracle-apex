...
output "KSQL" {
  value = tonumber(var.instance_count) >= 1 ? "sudo ksql via SSH  Access: ssh -i ~/keys/hackathon-temp-key.pem ec2-user@${join(",",formatlist("%s", aws_instance.kafka-rest.*.public_ip),)} " : "Confluent Cloud Platform on AWS is disabled" 
}
